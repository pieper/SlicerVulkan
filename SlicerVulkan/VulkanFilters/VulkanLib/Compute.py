...
class ComputeApplication(object):
    ...
    def __init__(self):
        ...
        self.WIDTH = 3200  # Size of rendered mandelbrot set.
        self.HEIGHT = 2400  # Size of renderered mandelbrot set.
        self.WORKGROUP_SIZE = 32  # Workgroup size in compute shader.

        self.enableValidationLayers = True
    ...
    def create(self):
        ...
        self.__bufferSize = self.pixel.buffer_info()[1] * self.pixel.itemsize * self.WIDTH * self.HEIGHT

        # The following methods are called in the same
        # order they are defined in the class, so you can read the
        # rest of the code in this file in order to see all
        # the steps required to use vulkan for computing
        self.createInstance()
    ...
    def run(self):
        ...
        # The former command rendered a the compute shader
        # to a buffer.
        # Reload that buffer to a numpy array
        st = time.time()

        self.reloadComputedBuffer()

        self.reloadTime = time.time() - st
    ...
    def findPhysicalDevice(self):
        ...
        slicer.modules.VulkanFiltersWidget.devices = devices
    ...
    def createBuffer(self):
        ...
        ###
        ### experiment - fill the buffer before executing
        ###
        pmappedMemory = vkMapMemory(self.__device, self.__bufferMemory, 0, self.__bufferSize, 0)

        volumeNode = slicer.util.getNode('MRHead')
        volumeArray = slicer.util.array('MRHead')
        flatHead = volumeArray.flatten()

        pa = numpy.frombuffer(pmappedMemory, numpy.float32)
        pa[:len(flatHead)] = numpy.array(flatHead, dtype='float32') / flatHead.max() 

        pa = pa.reshape((self.HEIGHT, self.WIDTH, 4))
        print(pa.shape)

        slicer.util.updateVolumeFromArray(volumeNode, pa)

        slicer.util.delayDisplay('filled from buffer',1)
    ...
    def createDescriptorSet(self):
        ...
        writeDescriptorSet = VkWriteDescriptorSet(
            ...
            dstBinding=10,  # write to the first, and only binding.
            descriptorCount=1,
            descriptorType=VK_DESCRIPTOR_TYPE_STORAGE_BUFFER,
